...
```bash
# 1. Clone the framework and the vendored Harbor fork
git clone https://github.com/SkillAudit/skillaudit.git
cd skillaudit
...
cp .env.example .env
# Edit .env: set ANTHROPIC_API_KEY, OPENAI_API_KEY, HARBOR_HOME, CODEX_HOME, SKILLAUDIT_ROOT
...
| `SKILLAUDIT_ROOT`       | All stages         | Project root used to resolve `INPUT_DIR`, `OUTPUT_DIR`, `JOB_DIR`. Defaults to one level above this pkg. |
...
[`skillaudit.github.io/artifacts/`](https://skillaudit.github.io/artifacts/).
...
released as a separate companion bundle** once the sanitization
...
The extension calls
`GET https://skillaudit.github.io/skillaudit/artifacts/api/lookup/<owner>__<repo>.json`
...
- **HTML listing** —
  [`/skillaudit/artifacts/api/lookup/`](https://skillaudit.github.io/skillaudit/artifacts/api/lookup/)
  (filterable table, one row per `<owner>__<repo>.json`).
- **Machine-readable index** —
  [`/skillaudit/artifacts/api/index.json`](https://skillaudit.github.io/skillaudit/artifacts/api/index.json).
...
A `CITATION.cff` entry is included for tooling that consumes it.

<!-- FIXME: replace the author placeholder in CITATION.cff with the real
     author list, and fill in the arXiv eprint ID once the preprint is live. -->
