...
2. **Looks up** a SkillPortrait report at
   ...
   `https://skillaudit.github.io/skillaudit/artifacts/api`, so the extension
...
`skillaudit_server_base_url`. The default is the public artifacts mirror at
`https://skillaudit.github.io/skillaudit/artifacts/api`.
...
| Permission | Why |
|------------|-----|
| `downloads` | Fetches the candidate skill bundle to a unique path under `~/Downloads/SkillAudit/` before submitting it for inspection. |
...
[`SkillAudit/skillaudit`](https://github.com/SkillAudit/skillaudit).
