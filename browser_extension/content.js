// SkillPortrait — marketplace overlay (clawhub.ai, skills.sh, skillsmp.com, ai-skills.io).
//
...
(() => {
  const LOG_PREFIX = "[SkillPortrait]";
  ...
  const DEFAULT_SERVER_BASE_URL = "https://skillaudit.github.io/skillaudit/artifacts/api";
  const SERVER_BASE_URL_STORAGE_KEY = "skillaudit_server_base_url";
  ...
  const DISMISSAL_STORAGE_KEY = "skillaudit_dismissed_v1";
  ...
  function classifyEfficiency(eff) {
    // Time-side score e^(t) ∈ [0, 1] per SkillAudit §4.4 formula (3) / (4).
  ...
  function classifyCost(cost) {
    // Token-side score e^(q) ∈ [0, 1] per SkillAudit §4.4 formula (3) / (4).
  ...
  function safetyStatus(score) {
    // Pass / Caution / Risky per SkillAudit §4.4 (V2 paper), threshold = 100 / 80.
      ...
      <div class="stb-empty">
        ...
        <div class="stb-empty-actions">
          <a class="stb-empty-cta"
             href="https://skillaudit.github.io/skillaudit/artifacts/api/lookup/"
  ...
  function patchHistoryNavigation() {
    if (window.__skillAuditMarketplaceHistoryPatched) return;
    window.__skillAuditMarketplaceHistoryPatched = true;
