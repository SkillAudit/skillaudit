"""
Codex smoke test — task generation.

Reads existing utility_scheme.json files, substitutes `/skill-name` → `$skill-name`
in each instruction, then uses Claude CLI to generate Harbor task files.

Output: codex_smoke/<skill_name>/tasks/<task_id>_wi_skills/
...
SMOKE_DIR   = ROOT / "codex_smoke"               # output
...
def _patch_instruction(scheme: dict) -> dict:
    """Replace `/skill-name` with `$skill-name` for Codex invocation."""
    skill_name = scheme.get("skill_name", "")
    instruction = scheme.get("instruction", "")
    patched = instruction.replace(f"`/{skill_name}`", f"`${skill_name}`")
